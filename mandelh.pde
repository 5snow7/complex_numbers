void mandel1(float w){
 float dx=w/width;float h=w*height/width;float dy=h/height;
  dy=h/height;
  loadPixels();
  comp z=new comp();
 ...
 float dy=h/height;
  ...
  for(int j=0;j<width;j++){
    for(int k=0;k<height;k++){
      z.real=-w/2+j*dx;z.imag=-h/2+k*dy;
      int loc=j+k*width;
    int ch=check(z,c);  //to get mandel set;change check(z,c) to check(z,z);
    if(ch==100){pixels[loc]=color(250,0,0);}
      else{pixels[loc]=color(ch*50%255,200,255-ch*50%255);}
    }}
    updatePixels();
 ...
 float dy=h/height;
  ...
  for(int j=0;j<width;j++){
    for(int k=0;k<height;k++){
      z.real=-w/2+j*dx;z.imag=-h/2+k*dy;
      int loc=j+k*width;
    int ch=checkSinz(z,c);  //to get mandel set;change check(z,c) to check(z,z);
    if(ch==100){pixels[loc]=color(250,0,0);}
      else{pixels[loc]=color(ch*50%255,200,255-ch*50%255);}
    }}
    updatePixels();
